    ...
    public partial class bill : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox2.Text != "" && textBox3.Text != "" && textBox4.Text != "" && dateTimePicker1.Text!="")
            {
                ...
                x.sqlConnection1.Open();
                SqlCommand cx = new SqlCommand("insert into Bill (Name_Patient,Cost,billdate) values(@Name_Patient,@Cost,@billdate)", x.sqlConnection1);
               // cx.Parameters.AddWithValue("@ID", textBox1.Text);
                cx.Parameters.AddWithValue("@Name_Patient", textBox3.Text);
                cx.Parameters.AddWithValue("@Cost", textBox4.Text);
                cx.Parameters.AddWithValue("@billdate", SqlDbType.Date).Value = dateTimePicker1.Value.Date;
                cx.ExecuteNonQuery();
